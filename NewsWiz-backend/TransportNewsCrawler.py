...
request = requests.get('https://sebenarnya.my/category/nasional/pengangkutan/')
...
page_url = 'https://sebenarnya.my/category/nasional/pengangkutan/'
...
collection_ref = db.collection("Transport")
...
while page_url:
    ...
    for news in main.find_all('a'):
        ...
        for temp in news.find_all('img'):
            ...
            doc_id = db.collection("Transport").document().id

            # Create a new document reference with the generated ID
            Transport = db.collection("Transport").document(doc_id)
