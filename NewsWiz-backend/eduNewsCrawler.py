...
request = requests.get('https://sebenarnya.my/category/nasional/pendidikan/')
...
page_url = 'https://sebenarnya.my/category/nasional/pendidikan/'
...
collection_ref = db.collection("Education")
...
while page_url:
    ...
    for news in main.find_all('a'):
        ...
        for temp in news.find_all('img'):
            ...
            doc_id = db.collection("Education").document().id

            # Create a new document reference with the generated ID
            Education = db.collection("Education").document(doc_id)
