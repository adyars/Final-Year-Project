...
request = requests.get('https://sebenarnya.my/category/sosial/kesihatan/')
...
page_url = 'https://sebenarnya.my/category/sosial/kesihatan/'
...
collection_ref = db.collection("Health")
...
while page_url:
    ...
    for news in main.find_all('a'):
        ...
        for temp in news.find_all('img'):
            ...
            doc_id = db.collection("Health").document().id

            # Create a new document reference with the generated ID
            Health = db.collection("Health").document(doc_id)
